...
import { Plus, Search, Users, ArrowRight, Filter, X, Code, Smartphone, Brain, Cloud, Shield, Palette, Star, PersonStanding, CircleUserRound, Mail, HeartHandshake, Check, User as UserIcon, Calendar as CalendarIcon, Venus, Mars, Genderqueer } from "lucide-react";
...
export default function Team() {
  ...
  const FilterSidebar = () => (
    <SheetContent side="right" className="w-full sm:max-w-sm">
      <SheetHeader>
        <SheetTitle className="flex items-center gap-2"><Filter className="w-5 h-5 text-primary" />Filtros Avançados</SheetTitle>
      ...
      <div className="py-4 space-y-6">
        ...
        <div>
          <Label className="mb-2 block font-semibold">Área Específica</Label>
        ...
        {activeFilters.category !== 'all' && (
          <div>
            <Label className="mb-2 block font-semibold">Especialização</Label>
        ...
        {activeFilters.specialization !== 'all' && (
          <div>
            <Label className="mb-2 block font-semibold">Competência</Label>
        ...
        <div>
          <Label className="mb-2 block font-semibold">Idade</Label>
          <div className="flex flex-wrap gap-2">
        ...
        <div>
          <Label className="mb-2 block font-semibold">Gênero</Label>
          <div className="flex flex-wrap gap-2">
    ...
    return (
      <Card key={member.id_usuario} className={cn("relative overflow-hidden w-full p-4 rounded-xl shadow-md transition-all duration-300 group", isComparisonMode ? "cursor-pointer hover:shadow-lg hover:border-primary/50" : "cursor-pointer hover:shadow-lg hover:-translate-y-1", isSelected && isComparisonMode && "border-2 border-primary ring-2 ring-primary/50")} onClick={() => isComparisonMode ? handleSelectMemberForComparison(member.id_usuario) : navigate(`/team/${member.id_usuario}`)}>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3"><Avatar className="w-12 h-12"><AvatarFallback className="bg-primary/10 text-primary font-semibold text-lg">{getInitials(member.nome)}</AvatarFallback></Avatar><div className="min-w-0"><h3 className="font-semibold text-lg text-foreground truncate">{member.nome}</h3><p className="text-sm text-muted-foreground truncate">{member.cargo_nome}</p></div></div>
