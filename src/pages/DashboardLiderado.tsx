...
// Removido: Helper para obter o ideal score de soft skills (agora vem do backend)
...
export default function DashboardLiderado() {
  ...
  useEffect(() => {
    if (profile?.role === 'LIDERADO' && profile.id_usuario && !profile.first_login) { // Só busca se não for o primeiro login
  ...
  const competenciasVersus = useMemo(() => {
    ...
    const softSkillsData = lideradoDashboardData.competencias
      ...
      .map(c => ({
        ...
        ideal: c.nota_ideal || 4.0, // Usar nota_ideal do backend, fallback para 4.0
      }));
  ...
  const categoryPerformanceData = useMemo(() => {
    ...
    return Array.from(categoryMap.entries()).map(([category, { soma, count }]) => ({
      category, atual: soma / count, ideal: 4.0, // Ideal para categorias técnicas ainda é 4.0
    }));
