...
export default function Compare() {
  ...
  const allSoftSkillCompetenciesFlat = useMemo(() => {
    ...
    softSkillTemplates.forEach(template => {
      template.competencias.forEach(comp => {
        ...
        if (!competencies.some(c => c.name === name)) {
  ...
  const sortedSoftSkillChipsData = useMemo(() => getSortedCompetencies(allSoftSkillCompetenciesFlat, selectedMembers, 'SOFT'), [allSoftSkillCompetenciesFlat, selectedMembers]);
  const sortedHardSkillChipsData = useMemo(() => getSortedCompetencies(allHardSkillCompetenciesFlat, selectedMembers, 'HARD'), [allHardSkillCompetenciesFlat, selectedMembers]);
  ...
  const hardSkillCategoriesForSelect = useMemo(() => technicalTemplate.map(cat => ({ id: cat.id_categoria, name: cat.nome_categoria })), []);

  const handleToggleSoftSkill = (skillName: string) => setSelectedSoftSkills(prev => prev.includes(skillName) ? prev.filter(s => s !== skillName) : [...prev, skillName]);
  const handleToggleHardSkill = (skillName: string) => setSelectedHardSkills(prev => prev.includes(skillName) ? prev.filter(s => s !== skillName) : [...prev, skillName]);

  const getRadarChartData = () => {
    if (selectedMembers.length < 1) return [];
    const combinedSelectedCompetencies = [...selectedSoftSkills, ...selectedHardSkills];
    if (combinedSelectedCompetencies.length === 0) return [];
          ...
          <div className="lg:w-1/3 flex-shrink-0 space-y-8">
            ...
            <div>
              ...
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 max-h-[200px] overflow-y-auto pr-2">
                      {presentHardSkillChips.map(skill => (
                        <Button key={skill.name} onClick={() => handleToggleHardSkill(skill.name)} className={cn("h-7 px-2.5 text-xs", selectedHardSkills.includes(skill.name) ? "bg-primary text-primary-foreground hover:bg-primary/90" : "border-primary text-primary bg-primary/10 hover:bg-primary/20")}>
                          {skill.name}
                        </Button>
                      ))}
                    </div>
                ...
                <AbsentHardSkillsCollapsible absentHardSkillChips={absentHardSkillChips} isHardSkillsAbsentCollapsed={isHardSkillsAbsentCollapsed} setIsHardSkillsAbsentCollapsed={setIsHardSkillsAbsentCollapsed} selectedHardSkills={selectedHardSkills} handleToggleHardSkill={handleToggleHardSkill} />
              </div>
            </div>

            {/* Filtros de Soft Skills */}
            <div>
              ...
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 max-h-[200px] overflow-y-auto pr-2">
                      {presentSoftSkillChips.map(skill => (
                        <Button key={skill.name} onClick={() => handleToggleSoftSkill(skill.name)} className={cn("h-7 px-2.5 text-xs", selectedSoftSkills.includes(skill.name) ? "bg-primary text-primary-foreground hover:bg-primary/90" : "border-primary text-primary bg-primary/10 hover:bg-primary/20")}>
                          {skill.name}
                        </Button>
                      ))}
                    </div>
                ...
                <AbsentSoftSkillsCollapsible absentSoftSkillChips={absentSoftSkillChips} isSoftSkillsAbsentCollapsed={isSoftSkillsAbsentCollapsed} setIsSoftSkillsAbsentCollapsed={setIsSoftSkillsAbsentCollapsed} selectedSoftSkills={selectedSoftSkills} handleToggleSoftSkill={handleToggleSoftSkill} />
              </div>
            </div>
          </div>
