...
const QUADRANT_COLORS: Record<NivelMaturidade, string> = {
  M1: "hsl(var(--color-accent))",      // Top-Left: High Behavioral, Low Technical
  M2: "hsl(var(--color-critical))",    // Bottom-Left: Low Behavioral, Low Technical
  M3: "hsl(var(--color-mid))",         // Bottom-Right: Low Behavioral, High Technical
  M4: "hsl(var(--color-brand))",       // Top-Right: High Behavioral, High Technical
};
        ...
        <ResponsiveContainer width="100%" height={400}>
          <ScatterChart margin={{ top: 20, right: 30, bottom: 40, left: 30 }}>
            ...
            <YAxis type="number" dataKey="eixo_y_comportamental" name="Comportamental" domain={[1, 4]} ticks={[1, 2, 2.5, 3, 4]} stroke={empty ? "hsl(var(--muted) / 0.5)" : "hsl(var(--foreground))"}>
              <Label value="Competências Comportamentais" angle={-90} offset={0} position="insideLeft" style={{ textAnchor: 'middle' }} />
            </YAxis>
            ...
            <ZAxis type="number" dataKey="z" range={[150, 400]} name="size" />
            ...
            <Scatter data={filteredMembers} fill="hsl(var(--color-brand))">
              {filteredMembers.map((entry) => (
                <Cell 
                  ...
                  opacity={selectedMemberId === null || selectedMemberId === entry.id_liderado ? 1 : 0.2}
